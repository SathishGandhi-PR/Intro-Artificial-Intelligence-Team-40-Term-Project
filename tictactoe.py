class TicTacToe():
    ...
    def gameState(self):
        ...
        for i in range(9):
            if(i%2==0):
                ...
                nextValue ='O'
            print(selectedAgent + " Placed it's " + nextValue + " at:")
            self.board()
            print("\n")
            if nextValue == 'X':
                if selectedAgent == "QLearning-Agent":
            ...
            if self.boardValues[idx] == " ":
                ...
                if self.gameOver() == 'X':
                    self.board()
                    print("We have a winner - Congratulations " + agent1)
                    break
                elif self.gameOver() == 'O':
                    self.board()
                    print("We have a winner - Congratulations " + agent2)
                    break
                elif self.gameOver() == 0:
                    self.board()
                    print("Its a Tie! we don't have a winner.")
                    break
                else:
                    continue
    ...
    def alphabeta(self, memory, currentAgent, alpha, beta):
        '''
        Alphabeta_minimax algorithm for choosing the best possible move towards
        winning the game
        '''
        if memory.isGameOverTemp(memory, currentAgent)== -1 or memory.isGameOverTemp(memory, currentAgent) == 1 or memory.isGameOverTemp(memory, currentAgent) == 0:
            return memory.isGameOverTemp(memory, currentAgent)
            ...
            maxValue = 0
            if currentAgent == "Minmax-AlphaBeta-Agent":
                for move in memory.positionsLeft():
                    memory.placeAgent(move, currentAgent)
    ...
    def expectimax(self, memory, currentAgent):
        ...
        if memory.isGameOverTemp(memory, currentAgent)== -1 or memory.isGameOverTemp(memory, currentAgent) == 1 or memory.isGameOverTemp(memory, currentAgent) == 0:
            ...
            maxValue = 0
            if currentAgent == "Expectimax-Agent":
                for move in memory.positionsLeft():
                    memory.placeAgent(move, currentAgent)
    ...
    def qLearning(self, board, QKey):
        """ If we have two in a row and the Third is available, take it. """
    ...
    def QLMove(self, board):
        ...
        """
        if(agent1 == "QLearning-Agent"):
            QKey = 'X'
            enemyKey = 'O'
        elif(agent2 == "QLearning-Agent"):
            QKey = 'O'
            enemyKey = 'X'
...
if __name__ == "__main__":
    ...
    game = TicTacToe()
    game.board()
    print("Select the Agent to Compete :")
    agentSelection = {1: 'Minmax-Agent', 2:'Minmax-AlphaBeta-Agent', 3:'Expectimax-Agent', 4:'QLearning-Agent'}
    for key, value in agentSelection.items():
        print(f"{key}: {value}")
    print("\n")
    val1 = int(input("Select Agent-1 to compete:"))
    agent1 = agentSelection.get(val1)
    print(agent1)
    print("\n")
    del agentSelection[val1]
    print("Select the Opponent Agent: ") 
    for key, value in agentSelection.items():
        print(f"{key}: {value}")
    print("\n")
    val2 = int(input("Select Agent-2 to compete:"))
